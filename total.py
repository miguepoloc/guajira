...
#Importamos la librería de subprocesos
import subprocess
...
def consulta_bdd(fecha_menor, fecha_mayor):
	...
	try:
		if fecha_mayor > fecha_menor:
			...
			for (id, sensor, fecha, tipo, valor, bateria, fecha_sms) in cursor:
...
def mejora_estacion(linea, fecha_sms):
	...
	for abc in s_estacion:
		...
		bdd(idx, 'prueba', f_h, abc, dic[abc], bat, fecha_sms)
...
def mejora(linea, fecha_sms):
	...
	#Guarda en la base de datos el id, cuál es, fecha, el tipo de sensor y valor
	print ("Guardando en base de datos")
	bdd(idx, 'prueba', f_h, tipo_sensor, valor, bat, fecha_sms)
	print ("---------------------------Fin del nodo----------------------------------\n")
